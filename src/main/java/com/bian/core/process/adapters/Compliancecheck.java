...
@Component
public class Compliancecheck implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private CompliancecheckRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Compliancecheck.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Compliancecheck.class.getName());
         }else{
