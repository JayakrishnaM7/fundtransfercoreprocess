...
public class Getaccountdetails implements JavaDelegate{
    ...
    @Autowired
    private AccountdetailsRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Getaccountdetails.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Getaccountdetails.class.getName());
         }else{
