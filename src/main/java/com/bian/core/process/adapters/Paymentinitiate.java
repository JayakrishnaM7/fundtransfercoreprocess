...
@Component
public class Paymentinitiate implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private FundtransferinitiateRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Paymentinitiate.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Paymentinitiate.class.getName());
         }else{
