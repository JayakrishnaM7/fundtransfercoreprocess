...
@Component
public class Verifyaccount implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private VerifyaccountRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Verifyaccount.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Verifyaccount.class.getName());
         }else{
