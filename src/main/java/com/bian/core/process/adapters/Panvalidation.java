...
@Component
public class Panvalidation implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private PanvalidationRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Panvalidation.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Panvalidation.class.getName());
         }else{
