...
@Service
public class FundtransferinitiateRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(FundtransferinitiateRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.fundtransferinitiate.hosturl}")
	private String url;
	
	@Value("${spring.api.fundtransferinitiate.context-path}")
	private String contextPath;

	@Value("${spring.api.fundtransferinitiate.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  FundtransferinitiateRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ FundtransferinitiateRestProcessor.class.getName());
