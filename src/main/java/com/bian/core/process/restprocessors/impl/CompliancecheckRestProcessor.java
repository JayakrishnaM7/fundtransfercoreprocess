...
@Service
public class CompliancecheckRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CompliancecheckRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.compliancecheck.hosturl}")
	private String url;
	
	@Value("${spring.api.compliancecheck.context-path}")
	private String contextPath;

	@Value("${spring.api.compliancecheck.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CompliancecheckRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CompliancecheckRestProcessor.class.getName());
