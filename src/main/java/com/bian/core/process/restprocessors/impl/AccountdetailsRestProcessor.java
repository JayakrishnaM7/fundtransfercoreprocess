...
@Service
public class AccountdetailsRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(AccountdetailsRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.accountdetails.hosturl}")
	private String url;
	
	@Value("${spring.api.accountdetails.context-path}")
	private String contextPath;

	@Value("${spring.api.accountdetails.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  AccountdetailsRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ AccountdetailsRestProcessor.class.getName());
