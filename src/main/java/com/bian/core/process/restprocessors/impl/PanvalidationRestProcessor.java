...
@Service
public class PanvalidationRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PanvalidationRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.panvalidation.hosturl}")
	private String url;
	
	@Value("${spring.api.panvalidation.context-path}")
	private String contextPath;

	@Value("${spring.api.panvalidation.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PanvalidationRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PanvalidationRestProcessor.class.getName());
