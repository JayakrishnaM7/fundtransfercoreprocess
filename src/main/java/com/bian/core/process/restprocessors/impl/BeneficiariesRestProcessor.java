...
@Service
public class BeneficiariesRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(BeneficiariesRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.beneficiaries.hosturl}")
	private String url;
	
	@Value("${spring.api.beneficiaries.context-path}")
	private String contextPath;

	@Value("${spring.api.beneficiaries.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  BeneficiariesRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ BeneficiariesRestProcessor.class.getName());
