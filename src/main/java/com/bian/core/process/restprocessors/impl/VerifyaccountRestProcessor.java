...
@Service
public class VerifyaccountRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(VerifyaccountRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.verifyaccount.hosturl}")
	private String url;
	
	@Value("${spring.api.verifyaccount.context-path}")
	private String contextPath;

	@Value("${spring.api.verifyaccount.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  VerifyaccountRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ VerifyaccountRestProcessor.class.getName());
